...
// the outline plus its interior, so the thumb reads solid at the same TEXT_SECONDARY tone.
...
// Canon 1147:15797 — a 20px glyph centred in a 28px tap target, filled TEXT_SECONDARY, gap 8.
// The chosen thumb switches to its FILLED glyph and the other one leaves.
const ICON_SIZE = 20;
const TAP_SIZE = 28;
...
function ThumbIcon({ variant, filled }: { variant: "up" | "down"; filled: boolean }) {
  ...
  return (
    <svg width={ICON_SIZE} height={ICON_SIZE} viewBox="0 0 20 20" fill="none">
      {paths.map((d, i) => (
        <path key={i} d={d} fill={TEXT_SECONDARY} />
      ))}
    </svg>
  );
}
