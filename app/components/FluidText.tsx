...
export function FluidText({ parts, style, trailing, trailingWidth = 0, layoutDuration = 220, align = "center" }: {
  parts: Part[];
  style?: CSSProperties;
  trailing?: ReactNode;
  ...
  const motion = useRef({ raf: 0, lastTime: 0, width: 0, target: 0, velocity: 0, omega: 28 });
  useLayoutEffect(() => () => { cancelAnimationFrame(motion.current.raf); motion.current.raf = 0; }, []);
  ...
  useLayoutEffect(() => {
    ...
    const layout = () => {
      ...
      if (!state.target || reduced.matches) {
      ...
      state.target = width;
      paint();
      if (reduced.matches) { cancelAnimationFrame(state.raf); state.raf = 0; return; }
  ...
  }, [parts, trailingWidth, layoutDuration, align]);
