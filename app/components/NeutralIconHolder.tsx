...
export default function NeutralIconHolder({ iconSrc, color, iconColor, size = 60, children }: {
  ...
  return (
    <div aria-hidden data-icon-holder="tile" style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{ position: "absolute", left: "27%", top: "27%", width: "46%", height: "46%", borderRadius: "50%", display: "grid", placeItems: "center", color, background: color ? `color-mix(in srgb, ${color} 20%, white)` : undefined, boxShadow: color ? `inset 0 0 0 1px color-mix(in srgb, ${color} 28%, transparent)` : undefined, transform: "perspective(160px) rotateY(-12deg) rotateX(6deg)", filter: "drop-shadow(0 1px 0 rgba(255,255,255,.6))" }}>
        {children ?? (iconSrc && (color
          ? <span style={{ ...glyph, backgroundColor: iconColor ?? "#fff", mask: `url("${iconSrc}") center / contain no-repeat`, WebkitMask: `url("${iconSrc}") center / contain no-repeat` }} />
