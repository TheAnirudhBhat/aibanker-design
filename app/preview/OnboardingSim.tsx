...
type Step =
  ...
  // Milestone 1 (R25): the three INDEPENDENT goal-creation flows — purchase,
  // budget, merchant/category tracking. One step, three sections, its own phase
  // machine (STEPS is memoised off config, so runtime branches live in a step).
  | { kind: "m1-goal-setup" }
...
function buildStepsForConfig(config: OnboardingConfig | undefined): Step[] {
  ...
  if (config?.betaIntentFirst) {
    ...
    return [
      ...
      { kind: "playground" },
      ...(goalAfterExplore ? goalNudge : []),
      // M1 goal setup sits where the goal ask begins. With the prescriptive flag
      // it renders nothing and auto-advances into the old preferences quiz.
      { kind: "m1-goal-setup" },
...
/** The chat's THINKING beat: a pulsing tertiary label, never a card (so it can't
    read as tappable) — canon is the plan cruncher's inline line. Shared by the
    cruncher and by the pause before each goal question (R21). */
function ThinkingLine({ label }: { label: string }) {
  return (
    <div className="flex items-center animate-chat-message-in" style={{ marginTop: SPACE_L, gap: 8, paddingTop: 4, paddingBottom: 4 }}>
      <p className="animate-thinking-pulse" style={{ ...typography.bodySmall, color: TEXT_TERTIARY }}>{label}</p>
    </div>
  );
}

// Ryan's text line - plain text, typewriter on first reveal, full text afterwards.
// thinkBeat (R21): lines whose CONTENT DEPENDS ON THE USER'S INPUT — computed
// replies, not predetermined follow-ups — hold a "Thinking" shimmer for a beat
// before they type. Already-revealed lines (active=false) never shimmer, so old
// messages re-render as plain full text.
function RyanLine({
  text,
  active,
  onDone,
  ...
  const [thoughtDone, setThoughtDone] = useState(!thinkBeat);
  useEffect(() => {
    if (thoughtDone || !active) return;
    const t = window.setTimeout(() => setThoughtDone(true), 900);
    return () => window.clearTimeout(t);
  }, [thoughtDone, active]);
  const thinking = thinkBeat && active && !thoughtDone;
  const displayed = useTypewriter(text, active && !thinking, onDone);
  if (thinking) return <ThinkingLine label="Thinking" />;
  return (
    <p
...
export default function OnboardingSim({
  ...
  // ══════════════════════════════════════════════════════════════════
  //  Goal setup, Milestone 1 (R25) — three independent creation flows
  //  Purchase · Budget · Tracking. One append-only transcript of beats, plus a
  //  phase that says which option rows are live. Beats are appended as the user
  //  moves, so the section reads back exactly like the rest of the chat.
  // ══════════════════════════════════════════════════════════════════
  const m1SeqRef = useRef(0);
  const [m1Beats, setM1Beats] = useState<M1Beat[]>([]);
  const [m1Phase, setM1Phase] = useState<M1Phase>("kind");
  const [m1Typed, setM1Typed] = useState<Record<number, boolean>>({});
  const [m1Kind, setM1Kind] = useState<M1GoalKind | null>(null);
  // Purchase answers
  const [m1What, setM1What] = useState<string | null>(null);
  const [m1Months, setM1Months] = useState(6);
  const [m1Target, setM1Target] = useState(150000);
  const [m1ExtraIncome, setM1ExtraIncome] = useState(0);
  const [m1ExtraObligation, setM1ExtraObligation] = useState(0);
  const [m1Lump, setM1Lump] = useState(0);
  const [m1Monthly, setM1Monthly] = useState<number | null>(null);
  // Budget answers
  const [m1Cap, setM1Cap] = useState<number | null>(null);
  // Tracking picks
  const [m1Tracked, setM1Tracked] = useState<string[]>([]);
  // A purchase goal parked mid flow (the switch beat) — comes back as an
  // incomplete plan, so nothing the user entered is lost.
  const [m1Draft, setM1Draft] = useState<string | null>(null);

  const m1GoalName = m1What ? M1_PURCHASE_WHAT.names[m1What] ?? "Your goal" : "Your goal";
  // What the plan actually needs each month, and what the month can spare.
  const m1Needed = Math.round((m1Target - m1Lump) / m1Months);
  const m1Spare = M1_SURPLUS + m1ExtraIncome - m1ExtraObligation;
  // Saving less doesn't fail the goal, it just moves the date (M1: no verdicts).
  const m1MonthsAt = (monthly: number) => Math.max(1, Math.ceil((m1Target - m1Lump) / monthly));

  /** Append beats and set the live phase. Every beat carries its own id so the
      newest Cosimo line types and the older ones render whole. */
  const m1Emit = useCallback((beats: Omit<M1Beat, "id">[], phase?: M1Phase) => {
    setM1Beats((prev) => [...prev, ...beats.map((b) => ({ ...b, id: ++m1SeqRef.current } as M1Beat))]);
    if (phase) setM1Phase(phase);
  }, []);

  /** The user picked or typed something: echo it, then play the next beats. */
  const m1Answer = useCallback((echo: string, beats: Omit<M1Beat, "id">[], phase?: M1Phase) => {
    setUserActionCount((c) => c + 1); // the reply leads its beat (R19 autoscroll)
    m1Emit([{ t: "echo", text: echo }, ...beats], phase);
  }, [m1Emit]);

  // ── Section 1: purchase ──────────────────────────────────────────────────
  const m1PickWhat = useCallback((id: string) => {
    const row = M1_PURCHASE_WHAT.rows.find((r) => r.id === id)!;
    setM1What(id);
    m1Answer(row.label, [{ t: "line", text: M1_PURCHASE_WHEN.ask }], "p-when");
  }, [m1Answer]);

  const m1PickWhen = useCallback((id: string) => {
    const row = M1_PURCHASE_WHEN.rows.find((r) => r.id === id)!;
    setM1Months(row.months);
    m1Answer(row.label, [{ t: "line", text: M1_PURCHASE_AMOUNT.ask }], "p-amount");
  }, [m1Answer]);

  const m1PickAmount = useCallback((id: string) => {
    const row = M1_PURCHASE_AMOUNT.rows.find((r) => r.id === id)!;
    setM1Target(row.amount);
    m1Answer(row.label, [
      { t: "line", text: `Got it. Before the number, let me show you what I can see.\n\nFrom your last three months, **${formatINR(M1_INCOME)} a month** is coming in.` },
      { t: "card", card: "income" },
      { t: "line", text: "Does that look right?" },
    ], "p-income");
  }, [m1Answer]);

  const m1PickIncome = useCallback((id: string) => {
    if (id === "more") {
      m1Answer("I have other income", [{ t: "line", text: "Where's it from?" }], "p-income-source");
      return;
    }
    m1Answer("That's right", [
      { t: "line", text: `And **${formatINR(M1_OBLIGATIONS)} a month** is already spoken for.` },
      { t: "card", card: "obligations" },
      { t: "line", text: "Anything I've missed?" },
    ], "p-obligations");
  }, [m1Answer]);

  const m1PickIncomeSource = useCallback((id: string) => {
    if (id === "connect") {
      m1Answer("Connect another account", [
        { t: "line", text: "I'll open that once we're done here. For now, tell me roughly what it adds each month." },
      ], "p-income-manual");
      return;
    }
    m1Answer("Enter it myself", [{ t: "line", text: "How much does it add each month?" }], "p-income-manual");
  }, [m1Answer]);

  const m1PickManualIncome = useCallback((id: string) => {
    const row = M1_MANUAL_INCOME_ROWS.find((r) => r.id === id)!;
    setM1ExtraIncome(row.amount);
    m1Answer(row.label, [
      { t: "line", text: `Noted, **${formatINR(M1_INCOME + row.amount)} a month** in total.\n\nAnd **${formatINR(M1_OBLIGATIONS)}** is already spoken for.` },
      { t: "card", card: "obligations" },
      { t: "line", text: "Anything I've missed?" },
    ], "p-obligations");
  }, [m1Answer]);

  const m1ShowNumber = useCallback((extraObligation: number, echo: string, lead?: string) => {
    const spare = M1_SURPLUS + m1ExtraIncome - extraObligation;
    m1Answer(echo, [
      ...(lead ? [{ t: "line" as const, text: lead }] : []),
      { t: "line", text: M1_FUNDING.ask },
    ], "p-funding");
    void spare;
  }, [m1Answer, m1ExtraIncome]);

  const m1PickObligations = useCallback((id: string) => {
    if (id === "add") {
      m1Answer("Add one I pay", [{ t: "line", text: "What is it?" }], "p-obligations-manual");
      return;
    }
    m1ShowNumber(0, "That's all of them");
  }, [m1Answer, m1ShowNumber]);

  const m1PickManualObligation = useCallback((id: string) => {
    const row = M1_MANUAL_OBLIGATION_ROWS.find((r) => r.id === id)!;
    setM1ExtraObligation(row.amount);
    m1ShowNumber(row.amount, row.label, `Added. That's **${formatINR(M1_OBLIGATIONS + row.amount)} a month** committed.`);
  }, [m1ShowNumber]);

  /** The facts beat: what the goal needs, what the month spares, and the ask. */
  const m1NumberBeats = useCallback((lump: number): Omit<M1Beat, "id">[] => {
    const needed = Math.round((m1Target - lump) / m1Months);
    const spare = M1_SURPLUS + m1ExtraIncome - m1ExtraObligation;
    return [
      { t: "line", text: `To get there you'd save **${formatINR(needed)} a month**.\n\nHere's what your last three months look like.` },
      { t: "card", card: "number", needed, spare },
      {
        t: "line",
        text: needed > spare
          ? `That's about ${formatINR(needed - spare)} more than the month spares right now. Your call on what to do next.`
          : `That fits inside what the month spares, with about ${formatINR(spare - needed)} left over.`,
      },
    ];
  }, [m1Target, m1Months, m1ExtraIncome, m1ExtraObligation]);

  const m1PickFunding = useCallback((id: string) => {
    const row = M1_FUNDING.rows.find((r) => r.id === id)!;
    if (id === "lump") {
      m1Answer(row.label, [{ t: "line", text: "How much can you put in now?" }], "p-funding-lump");
      return;
    }
    const lead = id === "sip"
      ? "Good to know. I can't see it, so I'll leave it out of the maths and you can count it as a buffer."
      : id === "later"
        ? "Noted. I'll plan without it, and you can tell me when it lands."
        : null;
    m1Answer(row.label, [...(lead ? [{ t: "line" as const, text: lead }] : []), ...m1NumberBeats(0)], "p-number");
  }, [m1Answer, m1NumberBeats]);

  const m1PickLump = useCallback((id: string) => {
    const row = M1_LUMP_ROWS.find((r) => r.id === id)!;
    setM1Lump(row.amount);
    m1Answer(row.label, m1NumberBeats(row.amount), "p-number");
  }, [m1Answer, m1NumberBeats]);

  /** Everything lands here: the autopay is the only thing M1 tracks. */
  const m1Commit = useCallback((monthly: number, echo: string) => {
    setM1Monthly(monthly);
    setPotFunded(true);
    setS2sPromptReady(true);
    m1Answer(echo, [
      { t: "line", text: `Done. **${formatINR(monthly)} a month** into ${m1GoalName}, starting this month.\n\nI'll only nudge you if a payment misses. ${m1GoalName} is on your dashboard now.` },
      { t: "card", card: "done" },
    ], "p-done");
  }, [m1Answer, m1GoalName]);

  const m1PickNumber = useCallback((id: string) => {
    if (id === "relook") {
      m1Answer("Relook at how much I save", [{ t: "line", text: "What feels doable each month?" }], "p-relook");
      return;
    }
    if (id === "cut") {
      m1Answer("Where could I cut?", [
        { t: "line", text: "These are your biggest movers. I'll leave the call to you." },
        { t: "card", card: "movers" },
      ], "p-number");
      return;
    }
    m1Commit(m1Needed, `Set up the ${formatINR(m1Needed)} autopay`);
  }, [m1Answer, m1Commit, m1Needed]);

  const m1PickRelook = useCallback((id: string) => {
    const row = M1_RELOOK_ROWS.find((r) => r.id === id)!;
    const months = m1MonthsAt(row.amount);
    setM1Monthly(row.amount);
    m1Answer(row.label, [
      { t: "line", text: `Then ${m1GoalName} takes about **${months} months** instead of ${m1Months}.\n\nWant me to set up the autopay at ${formatINR(row.amount)}?` },
    ], "p-relook-confirm");
  }, [m1Answer, m1GoalName, m1Months, m1MonthsAt]);

  // ── Section 2: budget ────────────────────────────────────────────────────
  const m1StartBudget = useCallback((echo: string, lead?: string) => {
    m1Answer(echo, [
      ...(lead ? [{ t: "line" as const, text: lead }] : []),
      { t: "line", text: `A budget is just a cap you set, no planning needed.\n\nFrom your last three months, **${formatINR(M1_INCOME)} a month** is coming in. Anything on top of that?` },
      { t: "card", card: "income" },
    ], "b-income");
  }, [m1Answer]);

  const m1PickBudgetIncome = useCallback((id: string) => {
    if (id === "more") {
      m1Answer("Add other income", [{ t: "line", text: "How much does it add each month?" }], "b-income-manual");
      return;
    }
    m1Answer("That's all of it", [
      { t: "line", text: "Here's what you've actually been spending, averaged over three months." },
      { t: "card", card: "spend" },
      { t: "line", text: "What do you want to cap it at each month?" },
    ], "b-cap");
  }, [m1Answer]);

  const m1PickBudgetManualIncome = useCallback((id: string) => {
    const row = M1_MANUAL_INCOME_ROWS.find((r) => r.id === id)!;
    setM1ExtraIncome(row.amount);
    m1Answer(row.label, [
      { t: "line", text: `Noted, **${formatINR(M1_INCOME + row.amount)} a month** in total.\n\nHere's what you've actually been spending, averaged over three months.` },
      { t: "card", card: "spend" },
      { t: "line", text: "What do you want to cap it at each month?" },
    ], "b-cap");
  }, [m1Answer]);

  const m1PickCap = useCallback((id: string) => {
    const row = M1_CAP_ROWS.find((r) => r.id === id)!;
    setM1Cap(row.amount);
    setPotFunded(true);
    setS2sPromptReady(true);
    m1Answer(row.label, [
      { t: "line", text: `Capped at **${formatINR(row.amount)} a month**. I'll track your spending against it and tell you where you stand.\n\nIt's on your dashboard now.` },
      { t: "card", card: "done" },
    ], "b-done");
  }, [m1Answer]);

  // ── Section 3: merchant / category tracking ─────────────────────────────
  const m1ToggleTrack = useCallback((id: string) => {
    setM1Tracked((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  }, []);

  const m1ConfirmTrack = useCallback(() => {
    const names = M1_TRACK_CANDIDATES.filter((c) => m1Tracked.includes(c.id)).map((c) => c.name);
    const list = names.length > 1 ? `${names.slice(0, -1).join(", ")} and ${names[names.length - 1]}` : names[0];
    setPotFunded(true);
    setS2sPromptReady(true);
    m1Answer(`Track ${list}`, [
      { t: "line", text: `Tracking **${list}**. Each one gets its own widget on your dashboard, and I'll keep the running totals.` },
      { t: "card", card: "done" },
    ], "t-done");
  }, [m1Answer, m1Tracked]);

  // ── The mid-flow switch: purchase parked as an incomplete plan ──────────
  const m1AskSwitch = useCallback(() => {
    m1Answer("Actually, I want to set a budget", [{ t: "line", text: M1_SWITCH.nudge }], "switch");
  }, [m1Answer]);

  const m1PickSwitch = useCallback((id: string) => {
    if (id === "finish") {
      // Back to whatever the purchase flow was waiting on.
      m1Answer("Finish this first", [{ t: "line", text: "Good. Picking up where we left off." }], m1ResumeRef.current);
      return;
    }
    setM1Draft(m1GoalName);
    m1Answer("Save it, set the budget", [{ t: "card", card: "draft" }]);
    m1StartBudget("", `Saved ${m1GoalName} as an incomplete plan, you can finish it any time.`);
  }, [m1Answer, m1GoalName, m1StartBudget]);

  /** The kind pick opens the section. */
  const m1PickKind = useCallback((id: M1GoalKind) => {
    setM1Kind(id);
    const row = M1_KIND_ROWS.find((r) => r.id === id)!;
    if (id === "purchase") {
      m1Answer(row.label, [{ t: "line", text: M1_PURCHASE_WHAT.ask }], "p-what");
      return;
    }
    if (id === "budget") {
      m1StartBudget(row.label);
      return;
    }
    m1Answer(row.label, [
      { t: "line", text: "Here's what you spend on most. Pick whatever you want me to keep an eye on." },
    ], "t-pick");
  }, [m1Answer, m1StartBudget]);
      ...
      <div ref={contentRef} className="flex flex-col" style={{ paddingLeft: SPACE_L, paddingRight: SPACE_L, paddingBottom: SPACE_L }}>
        ...
        {visibleSteps.map((step, i) => {
            ...
            return (
              <div key={`feas-${i}`}>
                <RyanLine
                  ...
                  // THE thinking moment of goal planning (R21): this is where the
                  // answers stop being collected and get worked out — amount +
                  // timeline + fixed spends into a monthly. The questions that
                  // gathered them ask straight away; only this pauses.
                  thinkBeat
              ...
              <div key={`bot-${i}`} ref={ref} className={isLast ? "animate-chat-message-in" : undefined}>
                {cosimoChat && step.dv === COSIMO_GREETING_1 ? (
                  ...
                  <RyanLine
                    text={botText}
                    active={isLast}
                    // The fixed-tenure ladder intro is COMPUTED from the user's goal
                    // (amount + deadline → monthly), so it thinks first; the scripted
                    // variants of this step are predetermined and land straight away.
                    thinkBeat={i === LADDER_INTRO_STEP_INDEX && hasFixedTenure}
                    onDone={onBotDone}
                  />
                )}
                  ...
                  {prefQuestions.map((q, k) => {
                    ...
                    return (
                      <div key={q.id} ref={isCurrent ? prefQuestionRef : undefined}>
                        {k > 0 && (
                          // The goal questions ask STRAIGHT AWAY (R21 pin): they're a
                          // predetermined run collecting answers, so pausing on each one
                          // padded the flow. The thinking belongs to the feasibility
                          // line, where those answers actually get worked out.
                          <RyanLine
                            text={q.text}
          ...
          if (step.kind === "plan-crunching") {
            ...
            return cruncherVisible ? <ThinkingLine key={`crunch-${i}`} label={cruncherStatus} /> : null;
          }
              ...
              <div key={`lump-sum-${i}`} className="animate-chat-message-in" style={{ marginTop: SPACE_M }}>
                ...
                {headStart != null && headStart > 0 && (
                  <RyanLine
                    ...
                    active={isLast}
                    thinkBeat // computed from the amount the user just gave
                    onDone={isLast ? advanceStep : undefined}
                  />
                )}
                {headStart === 0 && (
                  <RyanLine
                    ...
                    active={isLast}
                    thinkBeat // the reply still folds their answer into the monthly math
                    onDone={isLast ? advanceStep : undefined}
                  />
                )}
              </div>
                ...
                {tweakSubmitted && tweakDraft && (
                  <>
                    ...
                    <div style={{ marginTop: SPACE_M }}>
                      {/* reworking THEIR tweak — computed response, so it thinks first */}
                      <RyanLine text={reworkLine} active thinkBeat />
                    </div>
                  </>
                )}
              ...
              {conversational && freeTextBubbles.map((b, k) => b.step === i ? (
                <Fragment key={`free-step-${i}-${k}`}>
                  ...
                  <RyanLine
                    ...
                    thinkBeat // a contextual reply to free-typed input, never predetermined
                  />
                </Fragment>
              ) : null)}
